...
def read_table(table, dt):
    ...
    mysql_host = "hadoop105"
    mysql_schema = "data_supervisor"

    # 获取Mysql数据库连接
    connect = pymysql.connect(user=mysql_user, password=mysql_password, host=mysql_host, database=mysql_schema)
...
def one_alert(line):
    """
     ...
     :paramline: 一个等待通知的异常记录，{'notification_level':1, 'value_min': 0, 'value': 7, 'col': u'id', 'tbl': u'dim_user_info', 'dt':datetime.date(2021, 7, 16), 'value_max': 5}
    """
    ...
    request = urllib.request.Request(one_alert_host, body)
...
def mail_alert(line):
    ...
    mail_user = "429478948@qq.com"
    mail_pass = "lffahwhwfkkucagd"
...
def main(argv):
    """
     :paramargv: 系统参数，共三个，第一个为python脚本本身，第二个为告警方式，第三个为日期
    """
